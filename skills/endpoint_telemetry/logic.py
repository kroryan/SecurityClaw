...
def _validate_limit(limit: int) -> int:
    """Ensure limit is a safe positive integer before PowerShell string interpolation."""
...
def collect_network_connections(limit: int = 500) -> dict[str, Any]:
    if platform_name() == "linux":
        ...
        script = (
            ...
            "RemoteAddress,RemotePort,OwningProcess | ConvertTo-Json -Compress" % safe_limit
        )
...
def collect_network_defense(limit: int = 1000) -> dict[str, Any]:
    ...
    if platform_name() == "linux":
        ...
        safe_limit = _validate_limit(limit)
        scripts = {
            "neighbors": "Get-NetNeighbor | Select-Object -First %d IPAddress,LinkLayerAddress,State,InterfaceAlias,InterfaceIndex | ConvertTo-Json -Compress" % safe_limit,
            "routes": "Get-NetRoute | Select-Object -First %d DestinationPrefix,NextHop,RouteMetric,InterfaceAlias,InterfaceIndex | ConvertTo-Json -Compress" % safe_limit,
            "interfaces": "Get-NetAdapter | Select-Object -First %d Name,InterfaceDescription,Status,MacAddress,LinkSpeed,ifIndex | ConvertTo-Json -Compress" % safe_limit,
        }
...
def collect_software_inventory(limit: int = 2000) -> dict[str, Any]:
    ...
    if platform_name() == "linux":
        ...
        script = (
            ...
            "Select-Object -First %d DisplayName,DisplayVersion,Publisher,InstallLocation | ConvertTo-Json -Compress" % safe_limit
        )
        try:
...
def collect_security_posture(limit: int = 500) -> dict[str, Any]:
    ...
    if platform_name() == "linux":
        ...
        scripts = {
            ...
            "services": "Get-CimInstance Win32_Service | Select-Object -First %d Name,State,StartMode,PathName,DisplayName | ConvertTo-Json -Compress" % safe_limit,
        }
        for name, script in scripts.items():
            try:
                value = json.loads(_run(["powershell", "-NoProfile", "-NonInteractive", "-Command", script], timeout=45) or "[]")
